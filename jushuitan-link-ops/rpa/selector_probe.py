...
def build_argument_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Capture Jushuitan page elements for selector discovery.")
    parser.add_argument("--url", default="", help="Optional URL to open before capture.")
    parser.add_argument("--output", default="logs/selector_probe/selector_probe.json", help="JSON output path.")
...
def main() -> None:
    ...
    try:
        if args.url:
            driver.get(args.url)
        input("请完成登录并打开目标页面，然后按回车开始抓取 selector 探针结果...")
