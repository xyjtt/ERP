...
- The authoritative commit `b8c55de07eee352a1a304df810af27034d4ee80a` was fast-forwarded to the executor through a uniquely named bundle after the executor's Gitee fetch could not authenticate; the tracked worktree stayed clean and all pre-existing untracked files were preserved. Executor validation returned `668 tests OK`, compileall 0, 13 JSON files valid, PowerShell parser 0 and diff-check 0.
- After a fresh lease check (`crawler_active_count=0`, `leases=[]`, `requests=[]`, both `muke_lixiang` lock files absent), `YYDD-1688-Listing-Daily-Launcher` was registered at 08:00 under `SYSTEM/ServiceAccount/Highest`. A controlled `-Action start -SessionId 3` run made the existing worker `Running` and then `Ready` with `LastTaskResult=0`; TaskScheduler events 100/110/129/200/201/102 and scheduled log `20260806_131346.log` confirm the run. The summary was `duplicate_existing=1`, with no listing process, CDP 9306, new draft, execution or submit.
- The fresh read-only audit task confirms `workflow_state=offer_written_back`, `approval_status=approved`, latest `offer_written_back`, and Offer `1072868453052` / `https://detail.1688.com/offer/1072868453052.html`. Submit Saga `completed` is from the existing accepted reconciliation artifact; the duplicate run did not create a new Saga row. The launcher has registration history but no natural launcher run history yet, so the next 08:00 event remains a separate transport check.
...
- 日度候选、独立检查、draft Saga 和 submit Saga 都必须绑定 payload 中唯一既有 `draft_id`；缺失或存在多个草稿 ID 时一律 fail closed，不再允许 `authorized-new-draft` 等兜底身份，也不允许检查器参数改绑到另一草稿。
- `authorized_draft_rebuild_resumed` 不再是生产例外；没有显式 `--draft-id` 或已记录 `pending_draft_id` 的 draft 执行直接拒绝。
- draft 和 submit 使用不同且稳定的 operation key；旧版 `listing/<account>/<task>` 通用 Saga 终态不可信，不能据此短路新流程或认定 submit 已完成。
