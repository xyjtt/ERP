...
def require_unique_existing_draft_id(
    payload: Mapping[str, Any],
    *,
    allow_new: bool = False,
) -> str:
    """Return the single existing draft_id, or "" for a controlled new draft.

    ``allow_new=True`` permits a candidate that carries no historical draft_id
    (an opt-in marker, e.g. workflow.auto_create_draft) so the executor may
    create the first 1688 draft. Without the opt-in, the previous fail-closed
    behaviour is preserved (new draft creation stays disabled).
    """
    draft_ids = existing_draft_ids(payload)
    if len(draft_ids) != 1:
        if allow_new and not draft_ids:
            return ""
...
def build_listing_operation_key(
    *,
    ...
    if normalized_draft_id and not _DRAFT_ID.fullmatch(normalized_draft_id):
        raise ListingReviewError("listing operation requires a valid existing draft_id")
    # A new-draft candidate (auto_create_draft opt-in) has no draft_id yet;
    # the operation key is derived with an empty draft slot so the first
    # draft creation is still keyed and idempotent per task.
