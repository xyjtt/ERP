...
class BrowserRPA:
    ...
    def _run_publish_steps(self, steps: list[dict[str, Any]], context: dict[str, Any]) -> None:
        for step in steps:
            ...
            if action == "sku_table_rows":
                self._fill_sku_table_rows(step, context)
                self._pause(self.browser_config.get("action_wait_seconds", 0.5))
                continue
    ...
    def _fill_sku_table_rows(self, step: dict[str, Any], context: dict[str, Any]) -> bool:
        """SPU 合并多 SKU：发布页 #guid-skuTable 添加规格行并逐行填写
        （颜色/尺寸/单价/可售数量/单品货号）。单 SKU（<=1 行）保持原流程跳过。"""
        sku_rows = list(context.get("sku_rows") or [])
        if len(sku_rows) <= 1:
            return True
        if not self.driver:
            raise RuntimeError("Browser has not been opened.")
        rows_payload = [
            {
                ...
                "sale_price": str(row.get("sale_price") or "").strip(),
            }
            for row in sku_rows
    ...
    def _assert_draft_request_trace(
        ...
        if isinstance(response_json, dict):
            ...
            if expected_draft_id and response_draft_id != expected_draft_id:
                raise PublishSubmitError(
    ...
    def _collect_draft_submit_reapply_evidence(
        ...
        response_succeeded = (
            ...
            and (
                ...
                or (response_draft_id == expected_draft_id and request_draft_id == expected_draft_id)
            )
        )
        ...
        # For a brand-new draft the save request carries no draftId; the
        # response draftId is the identity of this save round, so bind the
        # contract to it (expected_draft_id is empty in that flow).
        effective_draft_id = expected_draft_id or response_draft_id
        ...
        if required_fields:
            evidence = {
                ...
                "save": {
                    ...
                    "request_draft_id": effective_draft_id,
    ...
    def _verify_saved_draft(self, publish_config: dict[str, Any], context: dict[str, Any]) -> None:
        ...
        if verification.get("require_title", False):
            actual_title = re.sub(r"\s+", " ", str(context.get("draft_title_value", "")).strip())
            ...
            if expected_title and actual_title != expected_title:
                # 1688 truncates titles longer than the platform limit (40 chars).
                # The truncated draft title is a legitimate platform behaviour.
                if not (
                    len(expected_title) > 40
                    and actual_title == expected_title[:40]
                ):
