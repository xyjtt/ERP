...
def parse_args() -> SmokeArgs:
    parser = argparse.ArgumentParser(description="Run worker smoke test on SQL Server.")
    ...
    parser.add_argument(
        "--adapter-mode",
        choices=["mock", "preview", "validate"],
        default="mock",
        help="Adapter mode for worker test. Use mock/preview/validate for safety.",
    )
...
def main() -> None:
    ...
    worker = WorkerService(
        ...
        max_iterations=3,
        on_task_dispatched=lambda task_id: print(f"worker_dispatched={task_id}"),
    )
            ...
            CreateTaskRequest(
                ...
                items=[
                    CreateTaskItemRequest(
                        ...
                        source_record_key=str(payload.get("variant_id") or "variant-worker-smoke").strip(),
                    )
                ],
            )
        )
        print(f"task_created={task.task_id}")
        print(f"task_status_before_worker={task.status.value}")
