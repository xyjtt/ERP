...
def parse_args() -> SmokeArgs:
    parser = argparse.ArgumentParser(description="Run control-plane execute bridge smoke test on SQL Server.")
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Run in preview mode instead of real execute. Useful for testing without actual 1688 execution.",
    )
...
def main() -> None:
    ...
    adapter_mode = "preview" if args.dry_run else "execute"
    enable_real_execution = False if args.dry_run else True

    print(f"adapter_mode={adapter_mode}")
    print(f"enable_real_1688_execution={enable_real_execution}")
    ...
    try:
        task = task_service.create_task(
            CreateTaskRequest(
                ...
                created_by="codex-execute-smoke-test",
                items=[
        ...
        print("execute_dispatch_smoke=ok")
        ...
        if reflow_events:
            print(f"reflow_event_id={reflow_events[0].event_id}")
            print(f"reflow_event_type={reflow_events[0].event_type}")
            print(f"reflow_idempotency_key={reflow_events[0].idempotency_key}")
    finally:
        if task and task.pk:
            try:
